...
def test_qa_uninitialized_models(test_client):
    mock_text = "Arthur Vance Arthur Vance Arthur Vance signed the document."
    with patch("app.services.document_service.DocumentService.extract_full_text", return_value=(mock_text, 1)):
        files = {"file": ("report.pdf", b"%PDF-1.4 sample", "application/pdf")}
        upload_resp = test_client.post("/documents", files=files)
        doc_id = upload_resp.json()["document_id"]
    ...
    with patch("app.services.model_service.ModelService.get_model_container", return_value=None):
        ...
        data = response.json()
        assert data["status"] == "error"
        assert "Model initialization failed" in data["message"]
